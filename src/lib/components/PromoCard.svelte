<script lang="ts">
	...
	let topThreePosts: Array<any> = [123];
	sortedPosts.then((data) => {
		topThreePosts = data
			.filter((post: any) => post.meta?.tags?.toLowerCase().includes(tag.toLowerCase()))
			.slice(0, 3);
	});
</script>
...
<style>
	...
	.promo-card::before {
		...
		filter: blur(8px) sepia(76%) hue-rotate(89deg) grayscale(17%) brightness(0.6);
		z-index: 1;
		opacity: 90%;
	}
	.promo-card::after {
